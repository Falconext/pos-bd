...
const FALCONEXT_VOLUME_PRICING: Record<
  string,
  [number, number, number, number]
> = {
  Emprendedor: [14.9, 13.9, 12.9, 11.9],
  Negocio: [34.9, 32.9, 29.9, 27.9],
  Corporativo: [59.9, 56.9, 52.9, 49.9],
};
      ...
      .$transaction(async (tx) => {
        ...
        if (billingProvider === 'QPSE') {
          if (!data.usuarioPse || !data.contrasenaPse) {
            throw new BadRequestException(
              'Para QPSE debes enviar usuarioPse y contrasenaPse.',
            );
          }
        }
