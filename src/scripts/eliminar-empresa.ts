...
async function main() {
  ...
  const confirmar = String(process.env.CONFIRMAR || '').trim().toUpperCase();
  ...
  console.log(`   id=${empresa.id}  RUC=${empresa.ruc}  ${empresa.razonSocial}`);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.detalleComprobante.deleteMany({ where: { comprobante: { empresaId } } });
    ...
    await tx.detalleGuiaRemision.deleteMany({ where: { guiaRemision: { empresaId } } });
    await tx.itemPedidoTienda.deleteMany({ where: { pedido: { empresaId } } });
    await tx.historialEstadoPedido.deleteMany({ where: { pedido: { empresaId } } });
