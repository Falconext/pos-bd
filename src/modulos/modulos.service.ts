...
export class ModulosService {
    ...
    async findByCodigo(codigo: string, producto?: string) {
        return this.prisma.modulo.findFirst({
            where: { codigo, ...(producto ? { producto: this.normalizeProducto(producto) } : {}) },
            include: {
                subModulos: { orderBy: { orden: 'asc' } },
            },
        });
    ...
    async update(id: number, data: UpdateModuloDto) {
        ...
        if (productoObjetivo !== this.normalizeProducto(modulo.producto)) {
            const planesConModulo = await this.prisma.planModulo.count({ where: { moduloId: id } });
            if (planesConModulo > 0) {
                throw new BadRequestException('No puedes cambiar el producto de un módulo asignado a planes');
            }
        }
