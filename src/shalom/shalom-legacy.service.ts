...
export interface ShalomAgenciaLegacy {
  terId: string;
  nombre: string;
  departamento: string;
  provincia: string;
...
export interface ShalomOrderInputLegacy {
 ...
 */
@Injectable()
export class ShalomLegacyService {
  private readonly logger = new Logger(ShalomLegacyService.name);
  private agenciasCache: ShalomAgenciaLegacy[] | null = null;
  ...
  async getAgencias(): Promise<{
    ...
    try {
      ...
      this.agenciasCache = acumulado.map((a): ShalomAgenciaLegacy => {
  ...
  async createOrder(input: ShalomOrderInputLegacy, empresaId?: number): Promise<any> {
    const sessionToken = await this.obtenerSessionToken(empresaId);
    try {
