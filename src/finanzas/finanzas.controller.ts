...
export class FinanzasController {
    ...
    listarIngresos(
        ...
        return this.finanzasService.listarIngresos(user.empresaId, fechaInicio, fechaFin, tipo);
    }

    @Post('ingresos-manuales')
    @Roles('ADMIN_EMPRESA', 'USUARIO_EMPRESA')
    crearIngreso(@User() user: any, @Body() body: { concepto: string; tipo: string; monto: number; fecha: string; descripcion?: string }) {
        return this.finanzasService.crearIngreso(user.empresaId, body);
    }

    @Patch('ingresos-manuales/:id')
    @Roles('ADMIN_EMPRESA', 'USUARIO_EMPRESA')
    actualizarIngreso(@User() user: any, @Param('id', ParseIntPipe) id: number, @Body() body: any) {
    ...
    listarEgresos(
        ...
        return this.finanzasService.listarEgresos(user.empresaId, fechaInicio, fechaFin, categoria);
    }

    @Post('egresos')
    @Roles('ADMIN_EMPRESA', 'USUARIO_EMPRESA')
    crearEgreso(@User() user: any, @Body() body: { categoria: string; etiqueta?: string; monto: number; fecha: string; descripcion?: string }) {
        return this.finanzasService.crearEgreso(user.empresaId, body);
    }

    @Patch('egresos/:id')
    @Roles('ADMIN_EMPRESA', 'USUARIO_EMPRESA')
    actualizarEgreso(@User() user: any, @Param('id', ParseIntPipe) id: number, @Body() body: any) {
