...
export class ComprobanteService {
  ...
  async crearFormal(
    ...
    const dataBase: any = {
      ...
      // Cobranza en campo: vendedor de campo atribuido (se muestra en lugar del
      // usuario que registró la venta). Denormalizado como en Pago.
  ...
  private async buildPdfBufferInformal(
    ...
    // Cuentas bancarias para el PDF (cotización/nota de venta y, si el formato
    // las activa, factura/boleta). Fuente primaria: tabla multi-cuenta; fallback: legacy.
  ...
  async enviarWhatsAppComprobante(
    ...
    // Se regenera siempre antes de enviarlo al cliente: reusar comp.s3PdfUrl
    // a ciegas mandaba un PDF cacheado desde la primera vez que se generó,
    // con la plantilla y el branding (marca blanca) de ese momento — aunque
    // "Descargar PDF" en el listado sí regenera y ya mostraba el formato y
    // la marca correctos. generarYSubirPdf reutiliza el buffer cacheado
    // salvo que se le pida force=true, como aquí.
    const pdfUrl = await this.generarYSubirPdf(id, context, true);
