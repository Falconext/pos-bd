...
export class VehiculoService {
    ...
    async create(empresaId: number, dto: CreateVehiculoDto) {
        ...
        if (exists) throw new ConflictException(`Ya existe un vehículo con la placa ${placa}`);
    ...
    async update(id: number, empresaId: number, dto: Partial<CreateVehiculoDto>) {
        const vehiculo = await this.prisma.vehiculo.findFirst({ where: { id, empresaId } });
        if (!vehiculo) throw new NotFoundException('Vehículo no encontrado');
    ...
    async createActa(vehiculoId: number, empresaId: number, usuarioId: number, dto: CreateActaDto) {
        const vehiculo = await this.prisma.vehiculo.findFirst({ where: { id: vehiculoId, empresaId } });
        if (!vehiculo) throw new NotFoundException('Vehículo no encontrado');
